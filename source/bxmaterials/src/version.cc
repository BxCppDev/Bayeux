// version.cc - Implementation of materials version struct
//
...
int version::get_major() {
  return static_cast<int>(MATERIALS_VERSION_MAJOR);
}

int version::get_minor() {
  return static_cast<int>(MATERIALS_VERSION_MINOR);
}

int version::get_patch() {
  return static_cast<int>(MATERIALS_VERSION_PATCH);
}
...
std::string version::get_version() {
  ...
  if (version.empty()) {
    ...
    stream << MATERIALS_VERSION_MAJOR << "."
...
bool version::is_at_least(int major, int minor, int patch) {
  if (MATERIALS_VERSION_MAJOR < major) return false;
  if (MATERIALS_VERSION_MAJOR > major) return true;
  if (MATERIALS_VERSION_MINOR < minor) return false;
  if (MATERIALS_VERSION_MINOR > minor) return true;
  if (MATERIALS_VERSION_PATCH < patch) return false;
  return true;
}
