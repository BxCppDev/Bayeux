			    ...
			    bool inherit_) const
  {
    ...
    if ( __composition.size () == 0)  out_ << "  :  <empty>"<< std::endl;
      ...
      {  
	     ...
	     {  
	       out_ << "(# atoms) "<< std::endl;	
	       for (composition_map_t::const_iterator i = __composition.begin (); i !=  __composition.end (); i++)
	          {
	          ...
	          j++;
	           ...
	           out_ << i_tree_dumpable::skip_tag<< atag << "'"<<entry.elt_ptr->get_name ()<< "'" <<" : "<<
	           entry.nb_of_atoms<< " " << (entry.owned? "(owned)": "(not owned)") <<std::endl;
	        }
	     }  
	    ...
	    {  
	       ...
	       for (composition_map_t::const_iterator i = __composition.begin (); i !=  __composition.end (); i++)
	          ...
	          j++;
	           string atag = du::i_tree_dumpable::TAG;
	           if (j == __composition.end ()) atag = du::i_tree_dumpable::LAST_TAG;   
	           out_ << i_tree_dumpable::skip_tag<< atag << "'";
		   if(entry.mat_ptr==0)            out_ <<entry.elt_ptr->get_name ();
		   else                                   out_ <<entry.mat_ptr->get_name ();
		   out_ << "'" <<" : "<<entry.weight<< " " << (entry.owned? "(owned)": "(not owned)") <<std::endl;
	        }
	     }  	
