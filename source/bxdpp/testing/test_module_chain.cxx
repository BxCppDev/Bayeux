  ...
  try
    {
      ...
      if (vm.count ("manager-config"))
        {     
          ...
          std::clog << DPP_DU::io::notice << "Using module manager configuration file '" 
                    << mgr_config << "'." << std::endl;
        }
      else
        {
      ...
      DPP_DU::properties::read_config (MM_config_file, MM_config);      
          ...
          while (source.has_next_record ())
            {             
              ...
              if ((print_modulo > 0) &&  (data_counter % print_modulo == 0))
                {
                  std::clog << DPP_DU::io::notice << "Event #" << data_counter << "\n";
                }
              data_counter++;
