...
namespace dpp {
  ...
  DPP_MODULE_REGISTRATION_IMPLEMENT(merger_module, "dpp::merger_module")

  // // The default GP label of the bank :
  // const std::string & merger_module::default_gp_label()
  // {
  ...
  // const std::string & merger_module::default_flag_name()
  // {
  //   static std::string label = "dpp::merger_module::flag_name";
  //   return label;
  // }

  // // Setter for the EH label of the module :
  // void merger_module::set_GP_label (const std::string & a_gp_label)
  // {
  ...
  // const std::string & merger_module::get_GP_label () const
  // {
  //   return _GP_label_;
  // }

  // // Setter for the label of the module :
  // void merger_module::set_flag_name (const std::string & a_flag_name)
  // {
  ...
  // // Getter for the label of the module :
  // const std::string & merger_module::get_flag_name () const
  // {
  //   return _flag_name_;
  // }
  ...
  // Constructor :
  merger_module::merger_module(datatools::logger::priority logging_priority_)
  ...
  {
    if (is_initialized()) merger_module::reset();
    return;
  }

  // Initialization :
  void merger_module::initialize(const datatools::properties & a_config,
  ...
  merger_module::process(::datatools::things & data_record_)
  {
    DT_THROW_IF(! is_initialized (),
                std::logic_error,
                "Module '" << get_name () << "' is not initialized !");
		_process_(data_record_);
	...
	void merger_module::_process_(datatools::things & data_record_)
	{
		if (accumulator_size() == 0) {
			_accumulator_.emplace_back(data_record_);
			return;
		}
		if (_must_be_merged_(data_record_)) {
			{
				datatools::things next;
				_accumulator_.emplace_back(next);
			}
		} else {
	...
	std::size_t merger_module::accumulator_size() const
	{
		return _accumulator_.size();
	}
