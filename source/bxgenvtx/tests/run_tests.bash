...
if [ "x${GENVTX_ROOT}" = "x" ]; then
    echo " ERROR: ${appname}: GENVTX_ROOT is not defined ! Abort !" >&2  
    my_exit 1
else
    if [ ! -d ${GENVTX_ROOT} ]; then
	echo " ERROR: ${appname}: GENVTX_ROOT directory does not exist ! Abort !" >&2  
	my_exit 1
    fi
    the_base_dir=${GENVTX_ROOT}
fi
...
tmp_test_dir=${GENVTX_ROOT}/tests/tmp
