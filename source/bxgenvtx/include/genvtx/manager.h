...
namespace genvtx {
  ...
  public: 
    ...
    void set_vg_name (const std::string & vg_name_); 
    ...
    /// Constructor
    manager (bool debug_ = false, bool debug_vg_ = false, bool verbose_factory_ = false);
  ...
  private: 
    ...
    bool _debug_;       //!< Manager debug flag
    bool _debug_vg_;    //!< Vertex generator dedicated debug flag

    genvtx::i_vertex_generator::factory_register_type _factory_register_; /// The factory
    bool _factory_preload_;
    bool _factory_debug_;    //!< Factory debug flag

    int         _random_seed_; //!< The seed of the embeded pseudo random number generator (PRNG)
    std::string _rng_id_;      //!< The ID of the embededpseudo random number generator (PRNG)
