...
namespace emfield {
  ...
  void geom_map::initialize(const datatools::properties & config_)
  {
    ...
    datatools::logger::priority lp
      = datatools::logger::extract_logging_configuration(config_, datatools::logger::PRIO_UNDEFINED, true);
  ...
  void geom_map::_construct(const datatools::properties & config_)
  {
    ...
    for (size_t i = 0; i < association_labels.size(); i++) {
      ...
      DT_THROW_IF(! _fields_manager_->has_field(field_name), std::logic_error,
