...
function do_run ()
{
	...
	export EMFIELD_TESTING_DIR=${prefix_test_dir}/testing
    fi

    if [ "x${EMFIELD_TESTING_DIR}" = "x" ]; then
	echo "ERROR: ${appname}: Missing EMFIELD_TESTING_DIR environment variable !"
	return 1
    fi
    if [ ! -d ${EMFIELD_TESTING_DIR} ]; then
	echo "ERROR: ${appname}: Directory '${EMFIELD_TESTING_DIR}' does not exists !"
	return 1
    fi
