...
namespace mctools {
  ...
  namespace ex02 {
    ...
    void simulated_data_inspector::display(const mctools::simulated_data & sd_)
    {
      ...
      if (sd_.has_step_hits("detector")) {
        // std::cerr << "DEVEL: simulated_data_inspector::display: " << "sd_.has_step_hits(\"detector\")" << '\n';
        // Build the display informations about the "detector" MC hits (if any) :
        int nsteps = sd_.get_number_of_step_hits("detector");
        for (int i = 0; i < nsteps; i++) {
          const mctools::base_step_hit & truth_hit = sd_.get_step_hit("detector", i);
          if (_dump_hits_) {
            truth_hit.tree_dump(std::clog, "Truth 'detector' hit : ", " ");
            std::clog << std::endl;
          }
          const geomtools::vector_3d & step_start = truth_hit.get_position_start();
          const geomtools::vector_3d & step_stop = truth_hit.get_position_stop();
          ...
          //geomtools::blur_spot detector_hit_sprite(3, 1 * CLHEP::mm);
          //detector_hit_sprite.set_errors(sprite_dim, sprite_dim, sprite_dim);
          geomtools::box detector_hit_sprite((step_stop - step_start).x(),
          ...
          // Label:
          std::ostringstream detector_hit_name_oss;
          detector_hit_name_oss << "detector_" << i;

          geomtools::display_data::display_item & detector_hit_DI
            = dd.add_static_item (detector_hit_name_oss.str(),
                                  "group::mc::detector_hit",
                                  step_color);

          geomtools::placement detector_hit_plcmt;
          detector_hit_plcmt.set_translation(0.5 * (step_stop + step_start));
          detector_hit_sprite.generate_wires(detector_hit_DI.wires, detector_hit_plcmt);
        }
      }
