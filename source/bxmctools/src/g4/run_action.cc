...
namespace mctools {
  ...
  namespace g4 {
    ...
    bool run_action::is_debug () const
    {
      return _debug_;
    }

    void run_action::set_debug (bool a_new_value)
    {
      _debug_ = a_new_value;
      return;
    }
    ...
    void run_action::set_use_run_header_footer (bool a_new_value)
    {
      if (is_initialized ())
        {
          throw logic_error ("mctools::g4::run_action::set_use_run_header_footer: Object is locked !");
        }
    ...
    {
      //cerr << "DEVEL: run_action::increment_number_of_processed_event: Entering...\n";
      _number_of_processed_events_++;
      //cerr << "DEVEL: run_action::increment_number_of_processed_event: number_of_processed_events="<< _number_of_processed_events << "\n";
      //cerr << "DEVEL: run_action::increment_number_of_processed_event: Entering...\n";
      return;
    }
    ...
    {
      //cerr << "DEVEL: run_action::increment_number_of_saved_event: Entering...\n";
      _number_of_saved_events_++;
      //cerr << "DEVEL: run_action::increment_number_of_saved_event: number_of_saved_events="<< _number_of_saved_events << "\n";
      //cerr << "DEVEL: run_action::increment_number_of_saved_event: Entering...\n";
      return;
    }
      ...
      if (is_initialized ())
        {
          ...
          message << "run_action::set_output_file: "
    ...
    {
      _debug_ = false;
      if ( getenv ("MCTOOLS_G4_RUN_ACTION_DEVEL") != NULL )
        {
          _debug_ = true;
        }
      if (_debug_)
        {
          std::clog << "DEBUG: mctools::g4::run_action::ctor: Done." << std::endl;
        }
      _initialized_ = false;
    ...
    run_action::~run_action ()
    {
      if (_debug_)
        {
          std::clog << "DEBUG: mctools::g4::run_action::dtor: Done." << std::endl;
        }
      return;
    }
      ...
      if (! has_number_events_modulo ())
        {
          ...
          /*
            ...
            {
              set_number_events_modulo (NUMBER_EVENTS_MODULO_DEFAULT);
            }
          */
        }
          ...
          if (! using_brio && a_config.has_key ("file.compression"))
            {
              ...
              if (ok)
              ...
              else
                {
                  ostringstream message;
                  message << "run_action::initialize: "
                          << "Invalid compression mode '" << compression << "' !";
                  throw logic_error (message.str ());
                }
            }
    ...
    void run_action::_build_run_footer ()
    {
      ...
      clog << "NOTICE: " << "mctools::g4::run_action::_build_run_footer: "
           << "Number of processed events = " << _number_of_processed_events_ << endl;
      if (_save_data_)
        {
          clog << "NOTICE: " << "mctools::g4::run_action::_build_run_footer: "
               << "Number of saved events     = " << _number_of_saved_events_ << endl;
        }
      return;
    }
    ...
    {
      bool devel = false;
      if (! devel) devel = is_debug ();
      cout << flush;
      clog << flush;
      clog << "NOTICE: " << "mctools::g4::run_action::BeginOfRunAction: "
           << "# Run " << a_run->GetRunID () << " is starting..." << endl;
      if (is_debug ()) dump (clog);
      _number_of_processed_events_ = 0;
      _number_of_saved_events_ = 0;
      ...
      if (get_manager ().has_simulation_ctrl ())
        {
          if (simulation_ctrl::g_devel) cerr << "DEVEL: " << "mctools::g4::run_action::BeginOfRunAction: "
               << "Check point." << endl;
          simulation_ctrl & SimCtrl
             = grab_manager ().grab_simulation_ctrl ();
          {
            if (simulation_ctrl::g_devel) cerr << "DEVEL: " << "mctools::g4::run_action::BeginOfRunAction: "
                 << "Acquire the lock..." << endl;
            boost::mutex::scoped_lock lock (*SimCtrl.event_mutex);
            if (simulation_ctrl::g_devel) cerr << "DEVEL: " << "mctools::g4::run_action::BeginOfRunAction: "
                 << "Wait for the event control to be available again for G4 run start and G4 event loop start ..." << endl;
            ...
            if (SimCtrl.event_availability_status == simulation_ctrl::ABORT)
              {
                clog << "WARNING: " << "mctools::g4::run_action::BeginOfRunAction: "
                     << "Detect a 'Abort' request from the external process !" << endl;
                 G4RunManager::GetRunManager ()->AbortRun ();
              }
            if (simulation_ctrl::g_devel) cerr << "DEVEL: " << "mctools::g4::run_action::BeginOfRunAction: "
                 << "Starting the G4 run..." << endl;
          }
        }
      /*************************************************************/
      ...
      if (_save_data_)
        {
          ...
          if (using_brio)
            {
              clog << "NOTICE: "
                   << "mctools::g4::run_action::BeginOfRunAction: "
                   << "Opening brio output data file '" << output_file_name << "'..." << endl;
              _brio_writer_.set_existing_file_protected (true);
              ...
              const string & store = _brio_plain_simulated_data_store_label_;
              // Future : could be "event_record" (using a things) :
          ...
          else
            {
              clog << "NOTICE: "
                   << "mctools::g4::run_action::BeginOfRunAction: "
                   << "Opening datatools serialization output data file '" << output_file_name << "'..." << endl;
      ...
      clog << "NOTICE: " << "mctools::g4::run_action::BeginOfRunAction: "
           << "# Run " << a_run->GetRunID () << " is started." << endl;
      return;
    }

    void run_action::EndOfRunAction (const G4Run * a_run)
    {
      clog << "NOTICE: " << "mctools::g4::run_action::EndOfRunAction: "
           << "# Run " << a_run->GetRunID () << " is stopping..." << endl;
      ...
      if (_save_data_)
        {
          if (is_debug ()) clog << "DEBUG: mctools::g4::run_action::EndOfRunAction: "
                                << "closing the data writer for run #"
                                << a_run->GetRunID () << "..." << endl;
      ...
      clog << "NOTICE: " << "mctools::g4::run_action::EndOfRunAction: "
           << "# Run " << a_run->GetRunID () << " is stopped." << endl;
      return;
    }
