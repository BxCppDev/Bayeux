...
namespace mctools {
  namespace g4 {
    ...
    void simulation_module::_initialize_manager(datatools::service_manager & smgr_)
    {
      ...
      geant4Simulation_ = std::make_unique<manager>();
      ...
      if(geant4SimulationController_.get() == nullptr) {
        geant4SimulationController_ = std::make_unique<simulation_ctrl>(*geant4Simulation_);
      ...
      {
        {
          ...
          std::unique_lock<std::mutex> lock(*geant4SimulationController_->event_mutex);
        ...
        // Wait for the release of the event control by the G4 process :
        {
          ...
          while (geant4SimulationController_->event_availability_status == simulation_ctrl::AVAILABLE_FOR_G4) {
            geant4SimulationController_->event_available_condition->wait(lock);
          }
          DT_LOG_TRACE(get_logging_priority(),
