...
namespace mctools {
  ...
  namespace g4 {
    ...
    private:
      ...
      std::string geometryServiceName_; //!< The name/label of the Geometry service to be accessed
      std::string simdataBankName_; //!< The name/label of the 'simulated_data' bank in the event record

      const geomtools::manager * geometryManagerRef_ = nullptr; //!< Non-mutable reference to the geometry manager

      manager_parameters geant4Parameters_; //!< The configuration parameters for the GEANT4 simulation manager
      std::unique_ptr<manager> geant4Simulation_; //!< The embedded GEANT4 simulation manager
      std::unique_ptr<simulation_ctrl> geant4SimulationController_; //!< The embedded control object for thread synchronization
