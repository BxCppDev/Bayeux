...
public:
  ...
  void compute_automatic_params ()
  {
    ...
    double p2 = experiment::params_t::complement (p1, p3, p4);
    get_param (4).set_value_no_check (p2);

    // recompute auto parameters 'p1+p3': 
    double p13 = p1 + p3;
    get_param (7).set_value_no_check (p13);

    // recompute auto parameters 'p1+p4': 
    double p14 = p1 + p4;
    get_param (8).set_value_no_check (p14);

    // recompute auto parameters 'p3+p4': 
    double p34 = p3 + p4;
    get_param (9).set_value_no_check (p34);

    // recompute auto parameters 'p1-p3': 
    double p1_3 = p1 - p3;
    get_param (10).set_value_no_check (p1_3);

    // recompute auto parameters 'p2-p4': 
    double p2_4 = p2 - p4;
    get_param (11).set_value_no_check (p2_4);
...
int main (int argc_, char ** argv_)
{
  try
      ...
      while (iarg < argc_)
	{
      ...
      experiment::params_t exp_params;
      exp_params.a  = 3.0;
      exp_params.b  = 50.0;
      exp_params.r  = 0.05;
      exp_params.p1 = 0.30;
      exp_params.p2 = 0.45;
      exp_params.p3 = 0.10;
      exp_params.p4 = experiment::params_t::complement (exp_params.p1,
						    exp_params.p2,
						    exp_params.p3);
      ...
      if (time_seed) 
	{
      ...
      start_params.p4 = experiment::params_t::complement (start_params.p1,
							  start_params.p2,
							  start_params.p3);

      // debug:
      mygsl::multidimensional_minimization::g_debug = debug;
	  ...
	  for (int i = 0; i < sys.get_number_of_params (); i++)
	    {
	      ...
	      cerr << "TEST: " << sys.get_param (i).get_name () << " ==> " << bv << endl;
	      sys.get_param (i).set_best_value (bv);
	    }
