...
namespace mygsl {
  ...
  { 
    return __type & TYPE_CONST; 
  }

  bool param_entry::is_free () const 
  { 
    return __type & TYPE_FREE; 
  }

  bool param_entry::is_auto () const 
  { 
    return __type & TYPE_AUTO; 
  }
  ...
  {
    return __type & TYPE_AUTO_COMPUTED;
  }
  ...
  bool param_entry::is_in_safe_range () const
  {
    ...
    if (has_limit ()) 
      {
	if (ok && has_min () && (get_value () < (__min + get_step ()))) ok = false;
	if (ok && has_max () && (get_value () > (__max - get_step ()))) ok = false;
      }
    return ok;
  }
  ...
  bool param_entry::is_in_range () const
  {
    ...
    if (has_limit ()) 
      {
	if (ok && has_min () && (get_value () < __min)) ok = false;
	if (ok && has_max () && (get_value () > __max)) ok = false;
      }
    return ok;
  }
  ...
  double param_entry::get_value_safe (double & dist_) const
  {
    ...
    if (! is_const ())
      {
	if (has_min () && (get_value () < __min))
	  {
	    val = __min;
	    dist_ = __min - get_value ();
	  }
	if (has_max () && (get_value () > __max))
	  {
	    val= __max;
	    dist_ = get_value () - __max;
	  }
      }
    return val;
  }
  ...
  void param_entry::set_value_no_check (double value_)
  {
    if (is_const ())
      {
	throw runtime_error ("param_entry::set_value_no_check: Cannot change value for this parameter is 'const'!");
      }
    __value = value_;
    if (is_auto ()) 
      {
  ...
  {
    __limit ^= param_entry::LIMIT_MAX;
    __max = param_entry::NO_MAX_VALUE; 
  }

  void param_entry::set_no_min ()
  {
    __limit ^= param_entry::LIMIT_MIN;
    __min = param_entry::NO_MIN_VALUE; 
  }
  ...
  void param_entry::set_min_max (double min_, double max_)
  {
    if (min_ >= max_)
      {
	ostringstream message;
	message << "param_entry::set_min_max: " 
  ...
  {
    double old_val = __value;
    set_value_no_check (value_);
    if (! is_in_range ()) 
      {
	__value = old_val;
	std::ostringstream message;
	message << "param_entry::set_value: ";
  ...
  {
    __name  = name_;  
    __type  = param_entry::TYPE_FREE;
    __limit = param_entry::LIMIT_NO;
    __min   = param_entry::NO_MIN_VALUE; 
    __max   = param_entry::NO_MAX_VALUE; 
    __value = param_entry::NO_VALUE;
    __step  = DEFAULT_STEP;
    reset_best_value ();
  }
  ...
  param_entry::make_auto (const std::string & name_, double step_)
  {
    ...
    pe.__type  = param_entry::TYPE_AUTO;
    pe.set_step (step_);
    return pe;
  }
			   ...
			   double value_)
  {
    ...
    pe.__type = param_entry::TYPE_CONST;
    return pe;
  }
  ...
  {
    param_entry pe (name_);
    pe.__type = param_entry::TYPE_FREE;
    pe.set_value (value_);
    pe.set_step (step_);
    return pe;
  }
