// version.cc - Implementation of mygsl version struct
//
...
int version::get_major() {
  return static_cast<int>(MYGSL_VERSION_MAJOR);
}

int version::get_minor() {
  return static_cast<int>(MYGSL_VERSION_MINOR);
}

int version::get_patch() {
  return static_cast<int>(MYGSL_VERSION_PATCH);
}
...
bool version::is_at_least(int major, int minor, int patch) {
  if (MYGSL_VERSION_MAJOR < major) return false;
  if (MYGSL_VERSION_MAJOR > major) return true;
  if (MYGSL_VERSION_MINOR < minor) return false;
  if (MYGSL_VERSION_MINOR > minor) return true;
  if (MYGSL_VERSION_PATCH < patch) return false;
  return true;
}
