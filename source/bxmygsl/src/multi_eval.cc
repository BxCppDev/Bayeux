...
namespace mygsl {
  ...
  {
    DT_THROW_IF (dim_ != get_dimension (), std::logic_error, "Invalid dimension!");
    return;
  }
  ...
  {
    DT_THROW_IF (! is_valid (x_), std::domain_error, "Value '" << x_ << "' is out of the evaluation domain !");
    return _eval(x_);
  }
  ...
  void unary_eval_from_multi::init (const multi_eval & multi_eval_, int i_, const vector<double> & params_)
  {
    ...
    set_index (i_);
    DT_THROW_IF (params_.size () != _multi_eval_->get_dimension (),
                 std::domain_error, "Invalid const parameters dimension !");
    _index_ = i_;
    _params_ = params_;
    return;
  }
