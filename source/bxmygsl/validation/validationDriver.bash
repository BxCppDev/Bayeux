...
APPNAME="mygsl/validationDriver"
...
if [ "x${mygsl_VALIDATION_TMP_DIR}" != "x" ]; then
    validation_tmp_dir="${mygsl_VALIDATION_TMP_DIR}"  
fi

if [ "x${validation_tmp_dir}" = "x" ]; then
    validation_tmp_dir="/tmp/${USER}/mygsl/validation"
fi
...
if [ "x${MYGSL_VERSION}" = "x" ]; then
    if [ "x${validation_setup_bash}" = "x" ]; then
	echo "ERROR: ${appname}: Missing mygsl bash setup script !" >&2
	return 1
    else
	    ...
	    do_mygsl_setup
	    if [ $? -ne 0 ]; then
		echo "ERROR: ${appname}: Could not setup 'mygsl' !" >&2
		return 1
	    fi
	fi
    fi	
...
validation_data_dir=${MYGSL_DATA_DIR}/validation/data
...
function do_run ()
{
	...
	echo "ERROR: ${appname}: Missing MYGSL_DATA_DIR environment variable !" >&2
	return 1
    fi
    if [ ! -d ${MYGSL_DATA_DIR} ]; then
	echo "ERROR: ${appname}: Directory '${MYGSL_DATA_DIR}' does not exists !" >&2
	return 1
    fi
...
EOF
    ...
    local list_of_validation_scripts=$(ls -1 ${MYGSL_DATA_DIR}/validation/scripts/*.bash 2> /dev/null)
...
*********************************************************************
Running mygsl validation script : '${script}'
*********************************************************************
EOF
	cat >> ${report_file} <<EOF
...
*********************************************************************
End of mygsl validation report file.  
*********************************************************************
EOF

    cat >> ${log_file} <<EOF
*********************************************************************
End of mygsl validation log file.  
*********************************************************************
EOF
