...
function do_clean()
{
    ...
    rm -f *.hist
    rm -f h*.gpm
    test -f test_mm.pars && rm -f test_mm.pars
    test -f test_mm.pars.200 && rm -f test_mm.pars.200
    test -f test_mm.hist && rm -f test_mm.hist
    test -f test_mm.sol && rm -f test_mm.sol
    test -f af.gpm && rm -f  af.gpm
}

function do_run()
{
    local BIN=${MYGSL_ROOT}/$(uname -s)-$(uname -m | sed 's/ //g')/bin/test_multidimensional_minimization
    local OPTS="-l -d --5000"
    rm -f test_mm.hist test_mm.pars test_mm.sol 
    #rm -f test_mm.hist test_mm.pars test_mm.sol
    local nloops=100
    #nloops=5
    local iloop=0
...
function do_ana_a()
{
    cat test_mm.pars.200 | cut -d' ' -f1 | gsl-histogram 2.9 3.1 20 > a.hist
    cat>ha.gpm<<EOF
...
function do_ana_b()
{
    cat test_mm.pars.200 | cut -d' ' -f2 | gsl-histogram 40.0 60.0 20 > b.hist
    cat>hb.gpm<<EOF
...
function do_ana_r()
{
    cat test_mm.pars.200 | cut -d' ' -f3 | gsl-histogram 0.14 0.16 20 > r.hist
    cat>hr.gpm<<EOF
...
function do_ana_p1()
{
    cat test_mm.pars.200 | cut -d' ' -f4| gsl-histogram 0.25 0.35 20 > p1.hist
    cat>hp1.gpm<<EOF
ferr(x)=x<1?1:sqrt(x)
set grid
...
function do_ana_p2()
{
    cat test_mm.pars.200 | cut -d' ' -f5| gsl-histogram 0.40 0.50 40 > p2.hist
    cat>hp2.gpm<<EOF
ferr(x)=x<1?1:sqrt(x)
set grid
...
function do_ana_p3()
{
    cat test_mm.pars.200 | cut -d' ' -f6| gsl-histogram 0.08 0.12 10 > p3.hist
    cat>hp3.gpm<<EOF
ferr(x)=x<1?1:sqrt(x)
set grid
...
function do_ana_p4()
{
    cat test_mm.pars.200 | cut -d' ' -f7| gsl-histogram 0.12 0.18 20 > p4.hist
    cat>hp4.gpm<<EOF
ferr(x)=x<1?1:sqrt(x)
set grid
...
function do_ana_f()
{
    cat test_mm.pars.200 | cut -d' ' -f8 | gsl-histogram 750 900 25 > f.hist
    cat>hf.gpm<<EOF
ferr(x)=x<1?1:sqrt(x)
set grid
set xrange [750:900]
...
function do_ana_af()
{
    ...
    cat test_mm.pars.200 | cut -d' ' -f1,8 > af.data
    cat>af.gpm<<EOF
