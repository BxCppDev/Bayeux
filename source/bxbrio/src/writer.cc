...
namespace brio {
  ...
  {
    DT_THROW_IF (serial_tag_.empty (), std::logic_error, "Missing serialization tag for store with label '"  << label_ << "' !");
  ...
  {
    DT_THROW_IF (! _allow_mixed_types_in_stores_, std::logic_error, "Stores with mixed data types are not allowed !");
                                   ...
                                   size_t buffer_size_)
  {
    ...
    DT_LOG_NOTICE(get_logging_priority(),"Create a new store with label '" << label_ << "'...");
    const store_info * ptr_si = _get_store_info (label_);
    DT_THROW_IF (ptr_si != 0, std::logic_error, "Store with label '" << label_ << "' already exists !");
    // create a new store:
    {
      store_info si;
  ...
  void writer::_at_open (const string & filename_)
  {
    ...
    if (extension != expected_extension) {
      DT_LOG_WARNING(get_logging_priority(),
                     "Using extension different from `" << expected_extension<< "' is not recommended !");
    }
    string mode = "RECREATE";
    _file = new TFile (_filename.c_str (), mode.c_str (), "ROOT file from the `brio' library");
    DT_THROW_IF (_file == 0, std::runtime_error, "Cannot open file '" << _filename << "' !");
    if (_file->IsOpen ()) {
      DT_LOG_NOTICE(get_logging_priority(), "File '" << _filename << "' has been successfully opened !");
    } else {
