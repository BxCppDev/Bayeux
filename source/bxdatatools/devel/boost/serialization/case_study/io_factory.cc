...
namespace boost_case_study {
  ...
  namespace serialization {
      ...
      else 
	{
      ...
      if (is_text () || is_xml ())
	{
	  if (g_debug) std::clog << "DEBUG: io_factory::__init_read: "
				 << "FPU: stream.imbue..." 
				 << std::endl;
	  __in->imbue (*__locale);
	}
	...
	{
#ifdef DATATOOLS_USE_FPU
	  if (g_debug) std::clog << "DEBUG: io_factory::__init_write_archive: "
    ...
    void io_factory::dump (std::ostream & out_, 
      ...
      if (! title_.empty()) 
	{
      ...
      out_ << indent << tag 
	   << "Mode  : " << hex << __mode << dec << endl;

      out_ << indent << tag 
	   << "is_read  : " << is_read() << endl;

      out_ << indent << tag 
	   << "is_write : " << is_write() << endl;

      out_ << indent << tag 
	   << "is_compressed : " << is_compressed () << endl;

      out_ << indent << tag 
	   << "is_uncompressed : " << is_uncompressed () << endl;

      out_ << indent << tag 
	   << "is_gzip : " << is_gzip () << endl;

      out_ << indent << tag 
	   << "is_bzip2 : " << is_bzip2 () << endl;
      ...
      out_ << indent << tag 
	   << "is_single_archive : " << is_single_archive () << endl;

      out_ << indent << last_tag
	   << "is_multi_archives : " << is_multi_archives () << endl;
    ...
    {
      io_factory::dump (out_, "io_factory:", "");
    }
    ...
    data_reader::~data_reader ()
    {
      ...
      if (io_factory::g_debug) 
	{
	  std::clog << "DEBUG: data_reader::~data_reader." << std::endl;
	}
    }
