...
// The serializable 'copyable_things' container :
...
public:
  ...
  void dump (ostream & = clog) const;
	...
	DATATOOLS_CLONEABLE_DECLARATION (A)
	// equiv : virtual datatools::utils::i_cloneable * clone (void) const;
...
const string A::SERIAL_TAG = "test_copyable_things::A";
...
DATATOOLS_CLONEABLE_IMPLEMENTATION(A)
/*
A * A::clone (void) const
{
	return datatools::utils::i_cloneable::clone (*this);
}
*/
...
public:
	...
	DATATOOLS_CLONEABLE_DECLARATION (B)
	// equiv :	virtual B * clone (void) const;
...
DATATOOLS_CLONEABLE_IMPLEMENTATION(B)
/*
B * B::clone (void) const
{
	return datatools::utils::i_cloneable::clone (*this);
}
*/
...
const string B::SERIAL_TAG = "test_copyable_things::B";
...
BOOST_CLASS_EXPORT_KEY2 (A, "test_copyable_things::A")
...
BOOST_CLASS_EXPORT_KEY2 (B, "test_copyable_things::B")
...
int main (int argc_, char ** argv_)
{
  ...
  try
    {
      clog << "Test program for class 'datatools::utils::copyable_things' !" << endl; 
      ...
      while (iarg < argc_)
        {
          ...
          iarg++;
			...
			if (out)
			{
				// declare the 'bag' instance as a 'copyable_things' container:
				datatools::utils::copyable_things bag ("bag1", "A bag with things in it");	
       	...
       	bag.add<B> ("b1", "The b1 object").set_index (7654321);
				B & b2 = bag.add<B> ("b2", "A permanent object", datatools::utils::copyable_things::constant);
				b2.set_index (6878);
				bag.add<B> ("b3");
				...
				catch (datatools::utils::bad_copyable_things_cast & x)
					{
				...
				string filename = "test_copyable_things.txt";
				clog << "Store 'copyable_things'..." << endl;
				if (xml)
					{
						filename = "test_copyable_things.xml";
					}				
			...
			if (in)
			{
				// declare the 'bag' instance as an empty 'copyable_things' container:
				datatools::utils::copyable_things bag;

				// now we load the 'bag' from a Boost archive :
				string filename = "test_copyable_things.txt";
				clog << "Load 'copyable_things'..." << endl;
				if (xml)
					{
						filename = "test_copyable_things.xml";
					}				
				...
				bag.tree_dump (clog, "Bag : ");
				
				cerr << "DEVEL: " << "Copy..." << endl; 
				datatools::utils::copyable_things bag2 = bag;
				bag2.tree_dump (clog, "Bag (copy) : ");
...
// end of test_copyable_things.cxx
/*
** Local Variables: --
** mode: c++ --
** c-file-style: "gnu" --
