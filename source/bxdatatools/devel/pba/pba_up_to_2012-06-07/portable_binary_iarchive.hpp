...
#include <boost/archive/portable_binary_archive_exception.hpp>
...
namespace archive {
	 ...
	 * in a portable_binary_archive_exception if loaded into an int on a 32 bit system.
	 ...
	 */
	class portable_binary_iarchive : public portable_iprimitive

		// the example derives from common_oarchive but that lacks the
		// load_override functions so we chose to stay one level higher
		, public boost::archive::basic_binary_iarchive<portable_binary_iarchive>
	...
	#endif
	{
		...
		void init(unsigned flags)
		{
			...
			// extract and check the magic header byte
	...
	public:
		...
		{ 
			...
			default: throw portable_binary_archive_exception(c);
			}
		}
		...
		load(T & t, dummy<3> = 0)
		{
			...
			if (get_flags() & no_infnan && !fp::isfinite(t))
				throw portable_binary_archive_exception(t);
		...
		#endif
			, boost::archive::basic_binary_iarchive<portable_binary_iarchive>(flags)
		{
			init(flags);
		}

	#if BOOST_VERSION >= 103400
		portable_binary_iarchive(std::streambuf& sb, unsigned flags = 0)
	...
	typedef POLYMORPHIC(portable_binary_iarchive) polymorphic_portable_binary_iarchive;
	#undef POLYMORPHIC
...
BOOST_SERIALIZATION_REGISTER_ARCHIVE(boost::archive::polymorphic_portable_binary_iarchive)
#endif
...
namespace boost { namespace archive {
	...
	template class basic_binary_iarchive<boost::archive::portable_binary_iarchive>;
...
#if BOOST_VERSION < 104000
	template class detail::archive_pointer_iserializer<boost::archive::portable_binary_iarchive>;
#else
	template class detail::archive_serializer_map<boost::archive::portable_binary_iarchive>;
	//template class detail::archive_serializer_map<boost::archive::polymorphic_portable_binary_iarchive>;
#endif
