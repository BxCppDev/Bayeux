...
namespace archive {
	 ...
	 */
	class portable_binary_oarchive : public portable_oprimitive

		// the example derives from common_oarchive but that lacks the
		// save_override functions so we chose to stay one level higher
		, public boost::archive::basic_binary_oarchive<portable_binary_oarchive>
...
//				save<unsigned>(archive_version);
	...
	public:
		...
		save(const T & t, dummy<3> = 0)
		{
			...
			default: throw portable_binary_archive_exception(t);
			}
		...
		#endif
			, boost::archive::basic_binary_oarchive<portable_binary_oarchive>(flags)
		{
			init(flags);
		}

	#if BOOST_VERSION >= 103400
		portable_binary_oarchive(std::streambuf& sb, unsigned flags = 0)
	...
	typedef POLYMORPHIC(portable_binary_oarchive) polymorphic_portable_binary_oarchive;
	#undef POLYMORPHIC
...
BOOST_SERIALIZATION_REGISTER_ARCHIVE(boost::archive::polymorphic_portable_binary_oarchive)
#endif
...
namespace boost { namespace archive {
	...
	template class basic_binary_oarchive<boost::archive::portable_binary_oarchive>;
...
#if BOOST_VERSION < 104000
	template class detail::archive_pointer_oserializer<boost::archive::portable_binary_oarchive>;
#else
	template class detail::archive_serializer_map<boost::archive::portable_binary_oarchive>;
	//template class detail::archive_serializer_map<boost::archive::polymorphic_portable_binary_oarchive>;
#endif
