/*****************************************************************************/
/**
 * \file quasi_portable_oarchive.hpp
...
namespace eos {
   ...
   *
   * This  archive  addresses integer  size,  endianness and  floating
   * point  types so  that data  can be  transferred  across different
   * systems. The  archive consists primarily of  three different save
   * implementations  for  integral types,  floating  point types  and
   * string types. Those functions  are templates and use enable_if to
   * be correctly selected for overloading.
   *
   * \note The class is based on the portable binary example by Robert
   *       Ramey and uses Beman Dawes endian library plus fp_utilities
   *       by Johan Rade.
   *
   */
  ...
  public:
     ...
     * sizes  to   the  archive  in   order  to  detect   tranfers  to
     * non-compatible platforms.
     *
     * We  could have  called basic_binary_oarchive::init  which would
     * create   the  boost::serialization   standard   archive  header
     * containing also the library version. Due to efficiency we stick
     * with our own.
     *
     */
      ...
      else
	{
	  ...
	  uint32_t archive_version_value = archive_version;
	  //save<uint32_t> (archive_version_value);
	  operator<<(archive_version_value);
    ...
    portable_oarchive(std::ostream& os, int portability_flag_ , unsigned flags)
#if BOOST_VERSION < 103400
      : portable_oprimitive(os, flags & boost::archive::no_codecvt)
#else
	: portable_oprimitive(*os.rdbuf(), flags & boost::archive::no_codecvt)
      ...
      else
	{
	   ...
	   *  here we must ensure to write a 32 bits word
	   */
	  uint32_t archive_version_value = archive_version;
	  //save<uint32_t> (archive_version_value);
	  operator<<(archive_version_value);
	...
	{
	  throw std::logic_error (std::string ("Strict portable mode is not supported yet ! Will it ever be ?"));
	}
      return;
    }
