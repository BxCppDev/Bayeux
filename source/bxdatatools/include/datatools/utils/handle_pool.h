 ...
 *
 */
#ifndef DATATOOLS_UTILS_HANDLE_POOL_H
#define DATATOOLS_UTILS_HANDLE_POOL_H
// Standard Library
#include <iostream>
#include <string>
#include <stdexcept>
...
void handle_pool<T>::resize(size_t size) {
  if (number_of_used_item_ > 0) {
    throw std::logic_error("datatools::utils::handle_pool<T>::resize: Cannot resize pool with items in use");
  }
  this->resize_impl(size);
} 
...
#endif // DATATOOLS_UTILS_HANDLE_POOL_H
