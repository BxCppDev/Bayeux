...
int main(int argc_, char ** argv_)
{
  ...
  if (vMap.count("help")) {
    std::cout << "Usage:\n"
              << " test_configuration_variant_service_2 [options]\n"
              << optPublic
              << "\n";
  } else {
    ...
    try {
      ...
      const dtc::variant_repository & vrep = app_var_serv.get_repository();
      for (const auto & re_pair : vrep.get_registries()) {
        const auto & vreg = re_pair.second.get_registry();
        vreg.tree_dump(std::clog, re_pair.first, "");
      }
