...
namespace datatools {
  ...
  void ocd_driver_params::print(std::ostream & out_) const
  {
    ...
    out_ << "  Interactive       = " << std::boolalpha << interactive << std::endl;
    ...
    out_ << "  DLLs = ";
    for (const std::string & dll_name : this->dlls) {
      out_ << " '" << dll_name << "'";
    }
    out_ << std::endl;
    out_ << "  Action      = '" << action << "'" << std::endl;
    out_ << "  Action options = ";
    for (const std::string & action_opt : this->action_options) {
      out_ << " '" << action_opt << "'";
    }
    out_ << std::endl;
  ...
  {
    auto logging = _params_.logging;
    // logging = datatools::logger::PRIO_DEBUG;
    _params_ = params_;
    _dll_loader_ = std::make_unique<datatools::library_loader>(_params_.dll_loader_config);
    DT_LOG_DEBUG(logging, "#DLLs = " << _params_.dlls.size());
    for (const std::string & dll_name : _params_.dlls) {
      DT_LOG_DEBUG(logging, "Loading DLL '" << dll_name << "'...");
