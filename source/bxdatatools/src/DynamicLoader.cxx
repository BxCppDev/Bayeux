...
#include <datatools/detail/Configure.hxx>
...
namespace datatools {
  ...
  {
    // std::cerr << "[DEVEL] DynamicLoader::OpenLibrary: libname='" << libname << "'" << std::endl;
    return DynamicLoader::OpenLibrary(libname, 0);
  }
}

#define KWSYS_SUPPORTS_SHARED_LIBS 1
...
namespace datatools {
  ...
  DynamicLoader::LibraryHandle DynamicLoader::OpenLibrary(const std::string& libname, int /*flags*/)
  {
    // std::cerr << "[DEVEL] DynamicLoader::OpenLibrary (TEST): libname='" << libname << "'" << std::endl;
    return 0;
  }
  ...
  DynamicLoader::SymbolPointer DynamicLoader::GetSymbolAddress(DynamicLoader::LibraryHandle /*lib*/, const std::string& /*sym*/)
  {
    return 0;
  }
...
namespace datatools {
  ...
  DynamicLoader::LibraryHandle DynamicLoader::OpenLibrary(
                                                          const std::string& libname, int flags)
  {
    // std::cerr << "[DEVEL] DynamicLoader::OpenLibrary (*NIX): libname='" << libname << "'" << std::endl;
    ...
    // std::cerr << "[DEVEL] DynamicLoader::OpenLibrary (*NIX): llFlags=" << llFlags << std::endl;
