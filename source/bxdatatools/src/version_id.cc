...
namespace datatools {
  ...
  namespace utils {
                           ...
                           const std::string & indent_) const
    {
      ...
      out_ << indent_ << tag << "Has major    : " << has_major ();
      if (has_major ())
        {
          out_ << " => Major = " << _major_;
        }
      out_ << std::endl;
      out_ << indent_ << tag << "Has minor    : " << has_minor ();
      if (has_minor ())
        {
          out_ << " => Minor = " << _minor_;
        }
      out_ << std::endl;
      out_ << indent_ << tag << "Has revision : " << has_revision ();
      if (has_revision ())
        {
          out_ << " => Revision = " << _revision_;
        }
      out_ << std::endl;
      out_ << indent_ << tag << "Has tag      : " << has_tag ();
      if (has_tag ())
        {
          out_ << " => Tag = '" << _tag_ << "'";
        }
      ...
      out_ << std::endl;
      out_ << indent_ << tag      << "Valid        : " << is_valid () << std::endl;
        ...
        for (int i = 0; i < 3; i++)
          {
            ...
            try 
              {
                if (i == 0) set_major (val);
                if (i == 1) set_minor (val);
                if (i == 2) set_revision (val);
              }
            catch (std::exception & x)
              {
                reset ();
