...
#ifndef __io_factory_h
#define __io_factory_h 1
...
namespace io_factory {
	...
	__read_next_tag ();
      ...
      void load (Data & data_)
      {
	i_serializable & i_ser=
	  static_cast<i_serializable &> (data_);
	...
	if (__writer->is_multi_archives ()) __writer->stop_archive ();
      ...
      void store (const Data & data_)
      {
	const i_serializable & i_ser=
	  static_cast<const i_serializable &> (data_);
