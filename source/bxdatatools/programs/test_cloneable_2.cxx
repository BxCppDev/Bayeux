...
struct data1 : public datatools::utils::i_cloneable,
...
int main (int argc_ , char ** argv_)
{
  ...
  try 
	...
	data1 * cd1 = datatools::utils::i_cloneable::clone_it (d1);
	cd1->upgrade ().print ();
	if (cd1 != 0) delete cd1;

	datatools::utils::i_caster<datatools::utils::i_cloneable, datatools::serialization::i_serializable> * clone_caster 
	  = d1.get_cloneable_to_serializable_caster ();
	datatools::utils::i_cloneable * d1_clone = d1.clone ();

	clog << "d1_clone typename is '" << typeid(d1_clone).name () << "' @ " << d1_clone << endl;

	datatools::serialization::i_serializable * d1_serial = clone_caster->cast (d1_clone);
