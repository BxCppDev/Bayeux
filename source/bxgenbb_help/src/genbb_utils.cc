...
namespace genbb
{
  ...
  {
    static std::unique_ptr<std::vector<std::string> > vptr(nullptr);
    if (vptr.get() == nullptr) {
      vptr.reset(new std::vector<std::string>());
      std::vector<std::string> & v = *vptr.get();
      v.reserve(100);
#if BAYEUX_WITH_BXDECAY0 == 1
      const auto & dbd_isotopes = bxdecay0::background_isotopes();
      for (const auto & name : background_isotopes) {
        v.push_back(name);
      }
#else
      // Embedded legacy Decay0:
      v.push_back("Ac228");
      v.push_back("Am241");
      v.push_back("Ar39");
      v.push_back("Ar42");
  ...
  {
    static std::unique_ptr<std::map<int,std::string> > mptr(nullptr);
    if (mptr.get() == nullptr) {
...
#if BAYEUX_WITH_BXDECAY0 == 1
      {
        ...
        for (const auto & rec : dbd_modes) {
          m[rec.first] = rec.second.description;
        }
      }
#else
      // Embedded legacy Decay0:
  ...
  {
    static std::unique_ptr<std::vector<int> > eptr(nullptr);
    if (eptr.get() == nullptr) {
...
#if BAYEUX_WITH_BXDECAY0 == 1
      {
        const auto & dbd_modes_with_esum_range = bxdecay0::dbd_modes_with_esum_range();
        for (const auto & mode : dbd_modes_with_esum_range) {
          e.push_back(static_cast<int>(mode));
        }
      }
#else
      // Embedded legacy Decay0:
