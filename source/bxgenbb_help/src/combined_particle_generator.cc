...
namespace genbb {
                                              ...
                                              bool compute_classification_)
  {
    if (_debug_)
      {
        clog << "debug: " << "genbb::combined_particle_generator::_load_next: "
             << "Entering..."
             << endl;
      }
    if (! _initialized_)
      {
        throw std::logic_error ("genbb::combined_particle_generator::_load_next: Generator is notlocked/initialized !");
      }
    event_.reset ();
    ...
    if (!_generators_info_[pg_index].pg->has_next ())
      {
        std::ostringstream message;
  ...
  {
    /*
    cerr << "DEVEL: " << "genbb::combined_particle_generator::_at_init_: "
         << "Entering..."
         << endl;
    */
    if (! has_external_random ())
      {
        clog << "NOTICE: " << "genbb::combined_particle_generator::_at_init_: "
             << "Initializing embeded PRNG..."
             << endl;
        _random_.init ("taus2", _seed_);
      }

    if (_generators_info_.size() == 0)
      {
        std::ostringstream message;
