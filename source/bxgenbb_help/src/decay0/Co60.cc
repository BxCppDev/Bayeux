...
namespace genbb
{
  namespace decay0
  {
               ...
               double tcnuc, double &tdnuc)
    {
      ...
      double thlev;
      double thnuc;
      primary_event::particles_col_type::reverse_iterator ipg1173 = event_.grab_particles().rend();
      primary_event::particles_col_type::reverse_iterator ipg1333 = event_.grab_particles().rend();
      ...
      if (p <= cg)
        {
          ...
          ipg1173 = event_.grab_particles().rbegin();
        }
      else if (p <= cg + cK)
        {
    ...
    label_13330:
      ...
      if (p <= cg)
        {
          decay0_gamma (prng_, event_, Egamma, tclev, thlev, tdlev);
          ipg1333 = event_.grab_particles().rbegin();
        }
      else if (p <= cg + cK)
        {
          decay0_electron (prng_, event_, Egamma - EbindK, tclev,
                           thlev, tdlev);
          decay0_gamma (prng_, event_, EbindK, 0., 0., tdlev);
        }
      else
      ...
      if (ipg1333 != event_.grab_particles().rend() 
          && ipg1173 != event_.grab_particles().rend()) {
        double p1333 = ipg1333->get_momentum().mag();
        double p1173 = ipg1173->get_momentum().mag();
      ...
      label_1:
        ...
        ipg1333->grab_momentum().set (p1333 * stet1 * cos (phi1), 
                                      p1333 * stet1 * sin (phi1),
                                      p1333 * ctet1); 
        ipg1173->grab_momentum().set (p1173 * stet2 * cos (phi2),
