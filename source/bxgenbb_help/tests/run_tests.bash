...
tmp_test_dir=${GENBB_HELP_ROOT}/tests/tmp
...
EOF
	...
	elif [ "${exe}" = "prog_DECAY0.f77.exe" ]; then
	    test -f out.genbb && rm -f out.genbb
	    echo -e "1\nSe82\n0\n1\n10\n1\nY\nout.genbb" | ${bin} >> tests.log 2>&1
	    if [ $? -ne 0 ]; then
		let error_count=error_count+1
		pkgtools__msg_notice "${exe} failed !"
	    fi
