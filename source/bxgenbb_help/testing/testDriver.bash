...
if [ ${debug} -ne 0 ]; then
	...
	echo "DEBUG: ${appname}: materials_resource_test_dir=${materials_resource_test_dir}"
	echo "DEBUG: ${appname}: geomtools_resource_test_dir=${geomtools_resource_test_dir}"
...
function do_run ()
{
	...
	export GENBB_HELP_TESTING_DIR=${prefix_test_dir}/testing
    fi

    if [ "x${GENBB_HELP_TESTING_DIR}" = "x" ]; then
	echo "ERROR: ${appname}: Missing GENBB_HELP_TESTING_DIR environment variable !"
	return 1
    fi
    if [ ! -d ${GENBB_HELP_TESTING_DIR} ]; then
	echo "ERROR: ${appname}: Directory '${GENBB_HELP_TESTING_DIR}' does not exists !"
	return 1
    fi
