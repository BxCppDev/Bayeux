...
if [ "x${CUTS_ROOT}" = "x" ]; then
    echo " ERROR: ${appname}: CUTS_ROOT is not defined ! Abort !" >&2  
    my_exit 1
else
    if [ ! -d ${CUTS_ROOT} ]; then
	echo " ERROR: ${appname}: CUTS_ROOT directory does not exist ! Abort !" >&2  
	my_exit 1
    fi
    the_base_dir=${CUTS_ROOT}
fi
...
tmp_test_dir=${CUTS_ROOT}/tests/tmp
