  ...
  try
      ...
      if (step1)
	{
	  ...
	  geomtools::physical_volume vessel_phys ("vessel_phys", 
						  ...
						  world_log, 
						  vessel_pl);
	  vessel_phys.parameters ().store ("author", "anonymous");
      ...
      if (step2)
	{
	  ...
	  gas_log.parameters ().store ("material.state", "gas");
	  gas_log.parameters ().store ("material.pressure", 1.0 * CLHEP::atmosphere);
	  gas_log.parameters ().store ("material.temperature", 300. * CLHEP::kelvin);
	  ...
	  electrode_log.parameters ().store ("material", "iron");
	  electrode_log.parameters ().store ("material.state", "solid");
	  electrode_log.parameters ().store ("visualization.visible", true);  
	  electrode_log.parameters ().store ("visualization.color", "red");  
	  ...
	  geomtools::physical_volume electrode_0_phys ("electrode_0", 
						       ...
						       gas_log, 
						       electrode_0_pl);
	  electrode_0_phys.parameters ().store ("potential", +1.0 * CLHEP::volt);

	  geomtools::physical_volume electrode_1_phys ("electrode_1", 
						       electrode_log, 
						       gas_log, 
						       electrode_1_pl);
	  electrode_1_phys.parameters ().store ("potential", -1.0 * CLHEP::volt);
