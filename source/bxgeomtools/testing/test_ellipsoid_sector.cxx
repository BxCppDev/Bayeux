...
int main(int argc_, char ** argv_)
{
  ...
  try {
    std::clog << "Test program for class 'geomtools::ellipsoid_sector'!" << std::endl;
    ...
    tmp_file.create("/tmp", "test_ellipsoid_sector_");

    geomtools::ellipsoid_sector ss;
    ss.set_x_radius(1.0);
    ss.set_y_radius(0.5);
    ss.set_z_radius(2.0);
    if (do_sector) {
    ...
    if (do_cut) {
      ss.set_bottom_z_cut(-1.25);
      ss.set_top_z_cut(+0.75);
    }
...

    {
      ...
      tmp_file.out() << "# ellipsoid_sector " << std::endl;
      uint32_t ss_options = geomtools::ellipsoid_sector::WR_NONE;
      ss_options |= geomtools::ellipsoid_sector::WR_BASE_GRID;
      ss_options |= geomtools::ellipsoid_sector::WR_BASE_LOW_ANGLE_SAMPLING;
      geomtools::gnuplot_draw::draw_ellipsoid_sector(tmp_file.out(),
                                                       ss_placement,
                                                       ss,
                                                       ss_options);
    ...
    {
      // Draw ellipsoid_sector (index 1):
      tmp_file.out() << "# ellipsoid_sector " << std::endl;
      uint32_t ss_options = geomtools::ellipsoid_sector::WR_NONE;
      ss_options |= geomtools::ellipsoid_sector::WR_BASE_GRID;
      ss_options |= geomtools::ellipsoid_sector::WR_BASE_GRID_HIGH_DENSITY;
      // ss_options |= geomtools::ellipsoid_sector::WR_ELLSEC_NO_START_ANGLE_EDGE;
      // ss_options |= geomtools::ellipsoid_sector::WR_ELLSEC_NO_STOP_ANGLE_EDGE;
      geomtools::gnuplot_draw::draw_ellipsoid_sector(tmp_file.out(),
                                                       ss_placement,
                                                       ss,
                                                       ss_options);
    ...
    if (draw) {
#if GEOMTOOLS_WITH_GNUPLOT_DISPLAY == 1
      Gnuplot g1;
      g1.cmd("set title 'Test geomtools::ellipsoid_sector' ");
      g1.cmd("set grid");
...

      {
        ...
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 0 title 'Ellipsoid Sector' with lines ";
        g1.cmd(plot_cmd.str());
...

      {
        std::ostringstream plot_cmd;
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 1 title 'Ellipsoid Sector' with lines ";
