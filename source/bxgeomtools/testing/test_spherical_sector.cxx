...
int main(int argc_, char ** argv_)
{
  ...
  try {
    std::clog << "Test program for class 'geomtools::spherical_sector'!" << std::endl;
    ...
    tmp_file.create("/tmp", "test_spherical_sector_");

    geomtools::spherical_sector ss;
    double r = 2.0;
    ss.set_radius(r);
    if (do_sector) {
      ss.set_start_theta(M_PI/6);
      ss.set_delta_theta(M_PI/3);
      ss.set_start_phi(M_PI/4);
      ss.set_delta_phi(0.2 * M_PI);
    }
    ...
    {
      // Draw spherical_sector (index 0):
      tmp_file.out() << "# spherical_sector " << std::endl;
      uint32_t ss_options = geomtools::spherical_sector::WR_NONE;
      ss_options |= geomtools::spherical_sector::WR_BASE_GRID;
      ss_options |= geomtools::spherical_sector::WR_BASE_LOW_ANGLE_SAMPLING;
      geomtools::gnuplot_draw::draw_spherical_sector(tmp_file.out(),
                                                     ss_placement,
                                                     ss,
                                                     ss_options);
    ...
    {
      // Draw spherical_sector (index 1):
      tmp_file.out() << "# spherical_sector " << std::endl;
      uint32_t ss_options = geomtools::spherical_sector::WR_NONE;
      ss_options |= geomtools::spherical_sector::WR_BASE_GRID;
      // ss_options |= geomtools::spherical_sector::WR_BASE_HIGH_ANGLE_SAMPLING;
      ss_options |= geomtools::spherical_sector::WR_SPHSEC_NO_START_THETA_EDGE;
      ss_options |= geomtools::spherical_sector::WR_SPHSEC_NO_STOP_PHI_EDGE;
      geomtools::gnuplot_draw::draw_spherical_sector(tmp_file.out(),
                                                     ss_placement,
                                                     ss,
                                                     ss_options);
    ...
    if (draw) {
#if GEOMTOOLS_WITH_GNUPLOT_DISPLAY == 1
      Gnuplot g1;
      g1.cmd("set title 'Test geomtools::spherical_sector' ");
      g1.cmd("set grid");
...

      {
        ...
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 0 title 'Spherical_Sector' with lines ";
        g1.cmd(plot_cmd.str());
...

      {
        std::ostringstream plot_cmd;
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 1 title 'Spherical_Sector' with lines ";
