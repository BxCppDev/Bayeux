...
int main (int argc_, char ** argv_)
{
  ...
  try {
    ...
    bool locate_out    = false;
    bool draw          = false; // Interactive plot
    unsigned int nsides = 5;
    ...
    while (iarg < argc_) {
      ...
      if (arg == "-O" || arg == "--out")       locate_out = true;
    ...
    tmp_file.create("/tmp", "test_polygonal_frustrum_");
    ...
    geomtools::polygonal_frustrum my_solid;
    ...
    // Compute some quantities:
    {
      ...
      std::clog << "Outer side surface = "
                << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_OUTER_SIDE) / CLHEP::mm2
                << " mm2" << std::endl;

      std::clog << "Top surface = "
                << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_TOP) / CLHEP::mm2
                << " mm2" << std::endl;

      std::clog << "Bottom surface = " << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_BOTTOM) / CLHEP::mm2
                << " mm2" << std::endl;

      if (do_hole) {
        std::clog << "Inner side surface = "
                  << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_INNER_SIDE) / CLHEP::mm2
                  << " mm2" << std::endl;
      }

      // if (do_sector) {
      //   std::clog << "Start angle side surface = "
      //             << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_START_ANGLE) / CLHEP::mm2
      //             << " mm2" << std::endl;
      //   std::clog << "Stop angle side surface = "
      //             << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_STOP_ANGLE) / CLHEP::mm2
      //             << " mm2" << std::endl;
      // }

      std::clog << "Full surface = "
                << my_solid.get_surface(geomtools::polygonal_frustrum::FACE_ALL) / CLHEP::mm2
                << " mm2" << std::endl;
    }
    ...
    // Draw the volume:
    {
      ...
      geomtools::gnuplot_draw::draw_right_polygonal_frustrum(tmp_file.out(),
                                                             my_solid_placement,
                                                             my_solid,
                                                             geomtools::polygonal_frustrum::WR_NONE
                                                             | geomtools::polygonal_frustrum::WR_BASE_BOUNDINGS
                                                             // | geomtools::polygonal_frustrum::WR_RPF_NO_TOP_FACE
                                                             // | geomtools::polygonal_frustrum::WR_RPF_NO_BOTTOM_FACE
                                                             // | geomtools::polygonal_frustrum::WR_RPF_NO_OUTER_FACE
                                                             // | geomtools::polygonal_frustrum::WR_RPF_NO_INNER_FACE
                                                             );
    ...
    // Locate points with respect to the volume:
    {
      ...
      //bool locate = locate_outer_side | locate_inner_side | locate_bottom | locate_top
        // | locate_start | locate_stop
        // | locate_bulk;
      size_t nshoots = 150000;
      size_t counts = 0;
      for (int i = 0; i < (int) nshoots; i++) {
        double dim2 = 0.5 * dim;
        geomtools::vector_3d position(0.5 * dim2 * ( 0.0 + 2.0 * drand48()),
                                      0.5 * dim2 * ( 0.0 + 0.1 * drand48()),
        ...
        if (locate_outer_side && my_solid.is_on_surface(position_c,
                                                  geomtools::polygonal_frustrum::FACE_OUTER_SIDE,
                                                  skin)) {
          ...
          counts++;
                                                                          ...
                                                                          geomtools::polygonal_frustrum::FACE_INNER_SIDE,
                                                                          skin)) {
          ...
          counts++;
                                                        ...
                                                        geomtools::polygonal_frustrum::FACE_TOP,
                                                        skin)) {
          ...
          counts++;
                                                           ...
                                                           geomtools::polygonal_frustrum::FACE_BOTTOM,
                                                           skin)) {
          ...
          counts++;
        } else if (locate_out && my_solid.check_outside(position_c, skin)) {
          geomtools::gnuplot_draw::basic_draw_point_with_color(tmp_file.out(), position, 3.0);
          counts++;
        }
      }
      ...
      tmp_file.out() << std::endl << std::endl;
      std::clog << "INFO: Locate points done." << std::endl;
    }
    ...
    // Draw the volume with high wires sampling:
    {
      ...
      tmp_file.out() << std::endl << std::endl;
      std::clog << "INFO: Intercept points done." << std::endl;
    }
