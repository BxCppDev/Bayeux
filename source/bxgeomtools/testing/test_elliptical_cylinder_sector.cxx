// test_elliptical_cylinder_sector.cxx
...
int main(int argc_, char ** argv_)
{
  ...
  try {
    std::clog << "Test program for class 'geomtools::elliptical_cylinder_sector'!" << std::endl;
    ...
    tmp_file.create("/tmp", "test_elliptical_cylinder_sector_");

    geomtools::elliptical_cylinder_sector ss;
    ss.set_x_radius(1.0);
    ss.set_y_radius(0.5);
    ss.set_z(2.0);
    if (do_sector) {
      ss.set_start_angle(M_PI/4);
      ss.set_delta_angle(1.25 * M_PI);
    }
    ...
    {
      // Draw ellipsoid sector (index 0):
      tmp_file.out() << "# elliptical_cylinder_sector " << std::endl;
      uint32_t ss_options = geomtools::elliptical_cylinder_sector::WR_NONE;
      ss_options |= geomtools::elliptical_cylinder_sector::WR_BASE_GRID;
      ss_options |= geomtools::elliptical_cylinder_sector::WR_BASE_LOW_ANGLE_SAMPLING;
      geomtools::gnuplot_draw::draw_elliptical_cylinder_sector(tmp_file.out(),
                                                       ss_placement,
                                                       ss,
                                                       ss_options);
    ...
    {
      // Draw elliptical_cylinder_sector (index 1):
      tmp_file.out() << "# elliptical_cylinder_sector " << std::endl;
      uint32_t ss_options = geomtools::elliptical_cylinder_sector::WR_NONE;
      ss_options |= geomtools::elliptical_cylinder_sector::WR_BASE_GRID;
      ss_options |= geomtools::elliptical_cylinder_sector::WR_BASE_GRID_HIGH_DENSITY;
      // ss_options |= geomtools::elliptical_cylinder_sector::WR_ELLTUBSEC_NO_START_ANGLE_EDGE;
      // ss_options |= geomtools::elliptical_cylinder_sector::WR_ELLTUBSEC_NO_STOP_ANGLE_EDGE;
      geomtools::gnuplot_draw::draw_elliptical_cylinder_sector(tmp_file.out(),
                                                       ss_placement,
                                                       ss,
                                                       ss_options);
...

    {
      ...
      for (int i = 0; i < (int) nshoots; i++) {
        geomtools::vector_3d pos(-3 + 6. * drand48(),
                                 -3 + 6. * drand48(),
                                 -0.5 + drand48());
    ...
    if (draw) {
#if GEOMTOOLS_WITH_GNUPLOT_DISPLAY == 1
      Gnuplot g1;
      g1.cmd("set title 'Test geomtools::elliptical_cylinder_sector' ");
      g1.cmd("set grid");
...

      {
        ...
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 0 title 'Elliptical tube sector' with lines ";
        g1.cmd(plot_cmd.str());
...

      {
        std::ostringstream plot_cmd;
        plot_cmd << "splot '" << tmp_file.get_filename()
                 << "' index 1 title 'Elliptical tube sector' with lines ";
