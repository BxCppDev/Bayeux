...
namespace geomtools {
  ...
  bool manager::is_plugin_a (const std::string & a_plugin_name) const
  {
    ...
    if (found == _plugins_.end ()) {
      ...
      message << "geomtools::manager::is_plugin_a<>: No plugin named '" 
              << a_plugin_name << "' !";
  ...
  T& manager::grab_plugin (const std::string & a_plugin_name)
  {
    ...
    if (found == _plugins_.end()) {
      std::ostringstream message;
      message << "geomtools::manager::grab_plugin<>: No plugin named '" 
              << a_plugin_name << "' !";
      throw std::logic_error(message.str());
    }
    ...
    if (ti != tf) {
      T tmp;
      std::ostringstream message;
      message << "geomtools::manager::grab_plugin<>: Requested plugin type '" 
