...
namespace geomtools {
  ...
  bool subtraction_3d::g_devel = false;

  std::string 
  subtraction_3d::get_shape_name () const
  {
  ...
  vector_3d 
  subtraction_3d::get_normal_on_surface (const vector_3d & position_) const
  {
    vector_3d normal;
    invalidate (normal);
    const shape_t & sh1 = get_shape1 ();
  ...
  bool 
  subtraction_3d::is_on_surface (const vector_3d & position_, 
				 int mask_, 
				 double skin_) const
  {
    const shape_t & sh1 = get_shape1 ();
				  ...
				  double skin_) const
  {
    ...
    if (debug)
      {
	std::clog << "subtraction_3d::find_intercept: shape1 is '" 
		  << sh3d1.get_shape_name () << "'" << std::endl;
	std::clog << "subtraction_3d::find_intercept: shape2 is '" 
	...
	if (debug)
	  {
	    std::clog << "subtraction_3d::find_intercept: from_i=" << from_i << std::endl;
	    std::clog << "subtraction_3d::find_intercept: child_dir_i=" << child_dir_i << std::endl;
	  }
	...
	do
	  {
		...
		message << "subtraction_3d::find_intercept: "
    ...
    if (debug)
      {
	std::clog << "subtraction_3d::find_intercept: ok1=" 
		  << intercept_ok[0] << std::endl;
	std::clog << "subtraction_3d::find_intercept: ok2="
