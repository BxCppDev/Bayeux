...
namespace geomtools {
  ...
  vector_3d
  intersection_3d::get_normal_on_surface (const vector_3d & a_position,
  ...
  // virtual
  face_identifier intersection_3d::on_surface(const vector_3d & a_position,
                                              ...
                                              double a_skin) const
  {
    ...
    for (uint32_t ipart = FIRST_PART; ipart <= SECOND_PART; ipart++) {
      ...
      if (ipart == FIRST_PART) {
        ...
        if (! sh3d2.check_outside(pos2, skin)) {
                                   ...
                                   double skin_) const
  {
    ...
    do {
      ...
      if (sh3d1.find_intercept(from1, dir1, fii1, skin)) {
        ...
        if (! sh3d2.check_outside(impact1_2, skin)) {
    ...
    do {
      ...
      if (sh3d2.find_intercept(from2, dir2, fii2, skin)) {
        ...
        if (! sh3d1.check_outside(impact2_1, skin)) {
...
*/
  ...
  void intersection_3d::generate_wires_self(wires_type & wires_,
