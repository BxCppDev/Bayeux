...
namespace geomtools {
  ...
  {
    double a = __r * cos (M_PI / __n_sides);
    return a;
  }
 
  double regular_polygon::get_side_length () const
  {
    double sl = 2 * __r * sin (M_PI / __n_sides);
    return sl;
  }

  double regular_polygon::get_perimeter () const
  {
    return get_side_length () * __n_sides;
  }
   
  double regular_polygon::get_surface () const
  {
    return get_apothem () * get_side_length () * __n_sides * 0.5;
  }
